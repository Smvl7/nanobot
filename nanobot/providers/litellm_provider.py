    ...
    async def chat(
        ...
        try:
            ...
            if "claude-sonnet-4.5" not in model and "claude-3-5-sonnet" not in model:
                ...
                # Remove provider-specific params if necessary (but LiteLLM usually handles this)
                # However, we need to ensure the key is set for Anthropic if not using OpenRouter
                # Assuming OpenRouter handles routing, or we have ANTHROPIC_API_KEY set.
                ...
                try:
                    ...
                    return LLMResponse(
                        content=f"Error calling LLM (Primary & Fallback failed): {str(e)} -> {str(e2)}",
                        finish_reason="error",
                    )
